...
def clean_baidu_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = [
    'smallMot', 'bigMot', 'bicyclist', 'motorcyclist', 'others', 'TrafficCone', 'pedestrian'
    ]
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    MAX_TRUNCATION = [0.15, 0.3, 0.5]
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            valid_class = 1
        elif (current_cls_name == "bicyclist"
              and "motorcyclist" == gt_name):
            valid_class = 0
        elif (current_cls_name == "motorcyclist"
              and "bicyclist" == gt_name):
            valid_class = 0
        else:
            valid_class = -1
        ignore = False
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, dataset_name=None):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        if dataset_name == "BaiduDataset":
            rets = clean_baidu_data(gt_annos[i], dt_annos[i], current_class, difficulty)
        else:
...
def eval_class_v3(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, dataset_name=dataset_name)
...
def get_baidu_kitti_official_eval_result(gt_annos,
                             dt_annos,
                             current_classes,
                             difficultys=[0, 1, 2],
                             z_axis=1,
                             z_center=1.0,
                             dataset_name="BaiduDataset"):
    """
    ...
    overlap_mod = np.array([[0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
                            [0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5]])
    overlap_easy = np.array([[0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
...
def get_baidu_coco_eval_result(gt_annos,
                         dt_annos,
                         current_classes,
                         z_axis=1,
                         z_center=1.0,
